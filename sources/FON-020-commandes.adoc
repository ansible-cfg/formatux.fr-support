...
Il est impossible pour un administrateur, quel que soit son niveau, de connaître toutes les commandes et options dans les moindres détails. Une commande a été spécialement conçue pour accéder en ligne de commande à un ensemble d’aides, sous forme d’un manuel : la commande man (« le man est ton ami »). indexterm:[man]
...
La commande **whatis** permet de faire une recherche par mot clef au sein des pages de manuel : indexterm:[whatis]
