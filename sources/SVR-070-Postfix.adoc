...
**indexterm2:[Postfix]**, dont le développement a été aidé par IBM, est apparu dès **1998**, afin de résoudre les problèmes de sécurité de Sendmail, tout en offrant une **administration beaucoup plus souple et modulaire**.
...
L'agent **indexterm2:[cleanup]** a ensuite pris en charge le message. Le destinataire d'origine, bob, n'étant pas **pleinement qualifié** et non conforme à la norme **RFC 822**, **cleanup** l'a fourni au démon **indexterm2:[trivial-rewrite]** (événement qui est non journalisé) pour permettre la réécriture de l'adresse de messagerie d'origine.
...
La commande **indexterm2:[mailx]** est une commande de traitement du courrier (un MUA) dont nous n'étudierons que la partie envoi du courrier.
...
| -l	 |  Force mailx à se 
lancer en mode interactif même lorsque l'entrée n'est pas un terminal. En
...
La commande **indexterm2:[swaks]** (SWiss Army Knife for Smtp) est un outil de test orienté transaction pour SMTP.
